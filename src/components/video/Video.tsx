...
// TODO: Ask Caro about ZPS Theme here
const selectedVideoVariants = {
  [GlobalVariants.Zps]: ['zep-bg-yellow-500'],
  [GlobalVariants.Cat]: ['zep-bg-yellow-500'],
};

const selectedVideoTextVariants = {
  [GlobalVariants.Zps]: ['zep-text-typography-light-70'],
  [GlobalVariants.Cat]: ['zep-text-secondary-default'],
};

const bgColorClass = cva([], {
  variants: {
    variant: bgColorVariants,
  },
  defaultVariants: {
...
const selectedVideoClass = cva([], {
  variants: {
    variant: selectedVideoVariants,
  },
  defaultVariants: {
    variant: GlobalVariants.Cat,
  },
...
const selectedVideoText = cva([], {
  variants: {
    variant: selectedVideoTextVariants,
  },
  defaultVariants: {
    variant: GlobalVariants.Cat,
  },
...
export const Video = ({ id, videos, variant }: VideoProps) => {
  ...
  const variantHandler = (v: GlobalVariants) => {
    switch (v) {
  ...
  return (
    <div id={id}>
      <Layout
        wrapperClassname={clsx(bgColorClass({ variant }), '!zep-mb-[0]')}
        className="zep-py-3 sm:zep-py-4 md:zep-py-5"
      >
        <HeaderLongComponent
          ...
          variant={variantHandler(variant as GlobalVariants)}
        />
      </Layout>
      <Layout className={videoComponentStyles}>
        <div className={clsx('zep-w-full zep-relative', videos?.length > 1 && 'md:zep-w-[80%]')}>
          <ResponsivePlayer label={selectedVideo.cookiesResetLabel} description={selectedVideo.cookiesLayerDescription}  thumbnailAlt={selectedVideo.thumbnailAlt} key={key} url={selectedVideo.url} thumbnail={selectedVideo.thumbnail} />
          {videos.length > 1 && (
            <div
              className={clsx(
                ...
                >
                  <div
                    ...
                    className={clsx(
                      ...
                      selectedVideo === v && selectedVideoClass({ variant }),
                    )}
                  >
                    <div className="zep-relative">
                      ...
                      <div className="zep-hidden zep-top-[0px] zep-absolute md:zep-flex zep-items-center zep-h-full zep-w-full zep-px-[8px]">
                        <h3
                          className={clsx(
                            'zep-typography-headlineSM-fluid-cqi zep-top-[0px] zep-line-clamp-2',
                            selectedVideo === v ? selectedVideoText({ variant }) : 'zep-text-typography-light-100 ',
                          )}
                        >
                          {v.title}
                        </h3>
                  ...
                  <h3
                    className={clsx(
                      'md:zep-hidden zep-typography-headlineSM-fluid-cqi zep-line-clamp-2',
                      selectedVideo === v ? selectedVideoText({ variant }) : 'zep-text-typography-light-100 ',
                    )}
                  >
                    {v.title}
        ...
        {videos.length > 1 && (
          <div className={clsx('zep-w-full md:zep-hidden md:zep-bg-[unset] md:zep-bg-none', bgColorClass({ variant }))}>
            {videos.map((v, index) => (
              <div
                style={{ transform: `translateY(-${16 * index}px)` }}
                key={uuidv4() + v.title}
